...
EnumDialogConfirm = Enum({ YES = "是", NO = "否", YES_WITH_ANNOTATION = "仲裁"})
...
local function __calculateVotingResult(initiator, voters, results, secretAnnotations)
...
local function __startVotingProcess(initiator)
    ...
    broadcastToAll(initiatorName.." 宣布胜利，正在进行投票...", DEFAULT_COLOR_WHITE)

    -- 向每个玩家展示投票选项
    ---@type table<PlayerInstance, string> voteResults = {}    -- 存储投票结果
    local voteResults = {}
    ---@type table<PlayerInstance, boolean> secretAnnotations = {}  -- 存储人工审核
    local secretAnnotations = {}
    local votesReceived = 0  -- 已收到的投票数
    for _, voter in ipairs(votingPlayers) do
        voter.showOptionsDialog(
            "请投票决定胜利 "..initiatorName.." 是否胜利",
            ...
            function(selectedText, selectedIndex, playerColor)
                ...
                if votesReceived == #votingPlayers then
                    __calculateVotingResult(initiator, votingPlayers, voteResults, secretAnnotations)
                end
            end
        )
    end
